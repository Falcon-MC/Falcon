#include "Block/Blocks/CoralFan.h"
...
FALCON_REGISTER_BLOCK(CoralFan, 185);

bool CoralFan::matches(const std::string &identifier) {
    return CoralBlock::isLiveCoral(identifier) && !CoralBlock::matches(identifier);
}

void CoralFan::onScheduledUpdate(ServerNetworkHandler &owner, Level &level, const Vector3i &position,
...
void CoralFan::onPlaced(ServerNetworkHandler &owner, ServerPlayer &player, const Vector3i &position,
...
void CoralFan::onNeighbourChanged(ServerNetworkHandler &owner, Level &level, const Vector3i &position,
                                       const BlockState &state) const {
